...
<div class="layout">
  ...
  <section>
    ...
    <table class="rides">
      <thead>
        <tr>
          <th class="sortable" class:active={sortKey === "start_time"} on:click={() => setSort("start_time")}>
            Datum {sortKey === "start_time" ? (sortDir === "asc" ? "↑" : "↓") : ""}
          </th>
          <th class="sortable" class:active={sortKey === "name"} on:click={() => setSort("name")}>
            Name {sortKey === "name" ? (sortDir === "asc" ? "↑" : "↓") : ""}
          </th>
          <th class="sortable" class:active={sortKey === "bike_name"} on:click={() => setSort("bike_name")}>
            Rad {sortKey === "bike_name" ? (sortDir === "asc" ? "↑" : "↓") : ""}
          </th>
          <th class="sortable num" class:active={sortKey === "distance_m"} on:click={() => setSort("distance_m")}>
            Distanz {sortKey === "distance_m" ? (sortDir === "asc" ? "↑" : "↓") : ""}
          </th>
          <th class="sortable num" class:active={sortKey === "moving_s"} on:click={() => setSort("moving_s")}>
            Dauer {sortKey === "moving_s" ? (sortDir === "asc" ? "↑" : "↓") : ""}
          </th>
          <th class="sortable num" class:active={sortKey === "avg_speed_ms"} on:click={() => setSort("avg_speed_ms")}>
            ⌀ Speed {sortKey === "avg_speed_ms" ? (sortDir === "asc" ? "↑" : "↓") : ""}
          </th>
          <th class="sortable num" class:active={sortKey === "avg_power"} on:click={() => setSort("avg_power")}>
            ⌀ Power {sortKey === "avg_power" ? (sortDir === "asc" ? "↑" : "↓") : ""}
          </th>
          <th class="sortable num" class:active={sortKey === "np_power"} on:click={() => setSort("np_power")}>
            NP {sortKey === "np_power" ? (sortDir === "asc" ? "↑" : "↓") : ""}
          </th>
          <th class="sortable num" class:active={sortKey === "avg_hr"} on:click={() => setSort("avg_hr")}>
            ⌀ HR {sortKey === "avg_hr" ? (sortDir === "asc" ? "↑" : "↓") : ""}
          </th>
          <th class="sortable num" class:active={sortKey === "elevation_gain_m"} on:click={() => setSort("elevation_gain_m")}>
      ...
      <tbody>
        {#each sortedRides as r}
          <tr>
            <td><Link to={`/rides/${r.id}`}>{fmtDateShort(r.start_time)}</Link></td>
            <td><Link to={`/rides/${r.id}`}>{r.name ?? "–"}</Link></td>
            <td>{r.bike_name ?? "–"}</td>
            <td class="num">{fmtKm(r.distance_m)}</td>
            <td class="num">{fmtDuration(r.moving_s ?? r.elapsed_s)}</td>
            <td class="num">{fmtKmh(r.avg_speed_ms)}</td>
            <td class="num">{fmtNum(r.avg_power, 0, " W")}</td>
            <td class="num">{fmtNum(r.np_power, 0, " W")}</td>
            <td class="num">{fmtNum(r.avg_hr, 0, " bpm")}</td>
            <td class="num">{fmtNum(r.elevation_gain_m, 0, " m")}</td>
          </tr>
        {/each}
      </tbody>
    </table>
